...
def load_fits_pair(input_file, target_file):
    ...
    with fits.open(input_file) as hdul:
        input_data = hdul[0].data  # Assume data is in the first HDU

    # Load target features
    target_file = target_file.numpy().decode("utf-8")
    with fits.open(target_file) as hdul:
        target_data = hdul[0].data  # Assume data is in the first HDU
...
# Split test_pairs into validation and test sets (50%/50% split in this example)
val_pairs, final_test_pairs = train_test_split(test_pairs, test_size=0.5, random_state=42)
...
print(f"Number of test pairs: {len(val_pairs)}")

if train_pairs:
    print("Example train pair:", train_pairs[0])
if test_pairs:
    print("Example test pair:", test_pairs[0])
if val_pairs:
    print("Example test pair:", val_pairs[0])
...
def Discriminator():
    ...
    inp = tf.keras.layers.Input(shape=[128, 128, 6], name='input_image')  # 6 channels for input image
    tar = tf.keras.layers.Input(shape=[128, 128, 1], name='target_image')  # 1 channel for target image

    x = tf.keras.layers.concatenate([inp, tar])  # (batch_size, 128, 128, 7)
    ...
    up1 = upsample(256, 4)(leaky_relu)
    up2 = upsample(128, 4)(up1)
    up3 = upsample(64, 4)(up2)

    up4 = upsample(32, 4)(up3)

    last = tf.keras.layers.Conv2D(1, 4, strides=1,
                                  padding='same',
                                  kernel_initializer=initializer)(up4)  # (batch_size, 128, 128, 1)
...
checkpoint_dir = f'./training_checkpoints{RUN}'
...
# Assuming you have PSNR, SSIM, and MSE functions:
@tf.function
def PSNR(x, y):
...
def fit(train_ds, val_ds, test_ds, steps, run=RUN): 
    # Take three instances from the test set for visualization
    example_input, example_target = next(iter(test_ds.take(1)))
    example_input = example_input[:3]  # Take the first three instances
    example_target = example_target[:3]  # Take the first three instances

    # Ensure target shape matches (3, 128, 128, 1)
    if len(example_target.shape) == 3:  # If the shape is (3, 128, 128)
        example_target = np.expand_dims(example_target, axis=-1)
    ...
    for step, (input_image, target) in train_ds.repeat().take(steps).enumerate():
        if (step) % 1000 == 0:
            ...
            print(np.shape(example_target))
            run_validation(val_ds, step)
            generate_images(generator, example_input, example_target, step, run)
...
def generate_images(model, test_inputs, tars, index, run=RUN):
    predictions = model(test_inputs, training=True)
    print("Prediction shape:", predictions.shape)
    print("Test shape:", test_inputs.shape)
    print("Target shape:", tars.shape)

    plt.figure(figsize=(15, 10))

    # Get the min and max values from the ground truth
    vmin, vmax = np.min(tars), np.max(tars)

    # Iterate over three instances
    for i in range(3):
        # Get individual ground truth and prediction
        target_img = tars[i:i+1]
        prediction_img = predictions[i:i+1]

        # Compute metrics for this pair
        psnr_value = PSNR(prediction_img, target_img).numpy()
        print(psnr_value)
        ssim_value = SSIM(prediction_img, target_img).numpy()
        print(ssim_value)
        mse_value = MSE(prediction_img, target_img).numpy()
        print(mse_value)
        uqi_value = calculate_uqi(target_img, prediction_img)
        print(f'uqi value: {uqi_value}')

        # Ground truth
        plt.subplot(3, 2, 2 * i + 1)
        plt.title(f"Ground Truth {i + 1}")
        plt.imshow(tars[i, ..., 0], cmap='plasma', vmin=vmin, vmax=vmax)
        plt.axis('off')
        plt.text(5, 10, f"Min: {vmin:.4f}\nMax: {vmax:.4f}", color="white", fontsize=10, bbox=dict(facecolor='black', alpha=0.5))

        # Prediction
        plt.subplot(3, 2, 2 * i + 2)
        plt.title(f"Prediction {i + 1}")
        plt.imshow(predictions[i, ..., 0], cmap='plasma', vmin=vmin, vmax=vmax)
        print(predictions[i, ..., 0])
        plt.axis('off')

                # Add metrics as text on the prediction
        metrics_text = (
            f"PSNR: {psnr_value[0]}\n"
            f"SSIM: {ssim_value[0]}\n"
            f"MSE: {mse_value}\n"
        ...
        plt.imshow(prediction_img[0, ..., 0], cmap='plasma', vmin=vmin, vmax=vmax)
        plt.axis('off')
        plt.savefig(f'prediction_{run}_{index}_{i + 1}.png', bbox_inches='tight', pad_inches=0)
        plt.close() # Close the figure to avoid display

    plt.tight_layout()
    plt.savefig(f'result{run}_{index}_cmap.pdf')


fit(train_dataset, val_dataset, test_dataset, steps=50000)